...
local progs = {
    term = "kitty -o linux_display_server=wayland",
    fm = "thunar",
...
for k, v in pairs({
    XCURSOR_SIZE = "48", HYPRCURSOR_SIZE = "48", QT_QPA_PLATFORM = "wayland"
}) do hl.env(k, v) end

hl.config({
    general = {
        gaps_in = 0, gaps_out = 0, border_size = 0, layout = "dwindle", resize_on_border = true, allow_tearing = false,
        col = {
...
-- Workspace rules
for _, r in ipairs({
    { 5, "scrolling", { direction = "down" } },
    { 2, "scrolling" },
    { "special:magic", "master" },
    { 3, "master" },
    { 2, "lua:grid" },
}) do
    hl.workspace_rule({
...
local m = "ALT"
local mk = function(k, c, o) hl.bind(m .. " + " .. k, c, o) end
...
-- Window management
mk("C", hl.dsp.window.close())
mk("V", hl.dsp.window.float({ action = "toggle" }))
mk("P", hl.dsp.window.pseudo())
mk("F", hl.dsp.window.fullscreen({ all = true }))
...
for _, r in ipairs({
    ...
    { "osu", { class = "^(osu!)$" }, workspace = "2 silent" },
    { "librewolf", { class = "^(librewolf)$" }, workspace = "1" },
    { "steam", { class = "^(steam)$" }, workspace = "4 silent" },
    { "kitty", { class = "^(kitty)$" }, workspace = "3" },
}) do
    hl.window_rule({
        name = r[1],
        match = r[2],
        suppress_event = r.suppress_event,
        no_focus = r.no_focus,
